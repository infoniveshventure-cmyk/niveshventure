    ...
    const interval = setInterval(() => {
      setStocks((prevStocks) =>
        prevStocks.map((stock) => {
          // Simulate realistic market price tick fluctuations (-0.5% to +0.5%)
          const deviationPercent = (Math.random() - 0.5) * 0.4; // max 0.2% change per tick
          ...
          // Keep history capped at 7 items
      ...
      // Reset flash class after 800ms
      setTimeout(() => {
        setStocks((current) => current.map((s) => ({ ...s, flash: null })));
      }, 800);

    }, 3000); // Update every 3 seconds for active trading feel

    return () => clearInterval(interval);
  }, []);

  return (
    <section className="relative py-12 md:py-24 bg-[#050914] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 md:mb-12 gap-4">
          <div>
            ...
            <p className="text-xs md:text-sm text-ink-muted mt-2 max-w-xl">
              Track global index performances and premium stock yields as calculated directly by official brokerage API nodes.
            </p>
          </div>
          <div className="flex items-center gap-2 text-xs text-ink-muted bg-white/5 border border-white/5 px-3 py-1.5 rounded-xl self-start md:self-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {stocks.filter((s) => s.isIndex).map((index) => {
            ...
            return (
              <motion.div
                key={index.symbol}
                className={`glass-card p-5 border transition-all duration-300 ${flashColor}`}
                layoutId={`card-${index.symbol}`}
              >
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-[10px] text-ink-muted uppercase font-semibold tracking-wider">{index.name}</span>
        ...
        <div className="glass-card border border-white/10 overflow-hidden">
          <div className="px-4 py-3 border-b border-white/10 bg-white/5 flex items-center justify-between">
            ...
            <span className="text-[10px] text-ink-muted">Simultaneous Real-time Updates</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs md:text-sm">
              <thead>
                <tr className="border-b border-white/5 text-[10px] text-ink-muted uppercase tracking-wider bg-white/[0.02]">
